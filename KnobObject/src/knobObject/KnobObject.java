...
public class KnobObject extends Application  {
//    private Slider knob;
  ...
  private Slider knobObject;

  @Override public void start(final Stage primaryStage) {
    // create a slider.
	knobObject = new Slider(0,100,50);
	knobObject.setBlockIncrement(0.1);
	knobObject.setId("knob");
	knobObject.getStyleClass().add("knobStyle");
    knobObject.setTooltip(new Tooltip(SLIDER_TOOLTIP));
 
    // bind a plain text label to the slider value.
    final Label text = new Label();
    text.setTooltip(new Tooltip(LABEL_TOOLTIP));
    text.setText(Math.round(knobObject.getValue()) + "");
    knobObject.valueProperty().addListener(new ChangeListener<Number>() {
    ...
    text.setOnMouseClicked(newLabelEditHandler(text, knobObject));
    knobObject.valueProperty().addListener(new ChangeListener<Number>() {
      @Override public void changed(ObservableValue<? extends Number> observableValue, Number oldValue, Number newValue) {
    ...
    intField.valueProperty().bindBidirectional(knobObject.valueProperty());
    intField.setPrefWidth(50);
 
    // layout the scene.
    Group root = new Group();
    root.setAutoSizeChildren(true);
    root.getChildren().addAll(knobObject, text, intField);
    knobObject.resizeRelocate(122,175,389,26);
    text.resizeRelocate(170,95,389,26);
    intField.resizeRelocate(160,125,389,26);
...
//    final Scene scene = new Scene(controls);
    ...
    scene.getStylesheets().add(KnobObject.class.getResource("knobObject.css").toExternalForm());
